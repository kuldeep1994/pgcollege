...
<div class='admin-box'>
    ...
    <?php echo form_open($this->uri->uri_string(), 'class="form-horizontal"'); ?>
        ...
        <fieldset class='form-actions'>
            <input type='submit' name='save' class='btn btn-primary' value="<?php echo lang('fee_stucture_action_edit'); ?>" />
            <?php echo lang('bf_or'); ?>
            <?php echo anchor(SITE_AREA . '/master/fee_stucture', lang('fee_stucture_cancel'), 'class="btn btn-warning"'); ?>
            
            <?php if ($this->auth->has_permission('Fee_Stucture.Master.Delete')) : ?>
                <?php echo lang('bf_or'); ?>
                <button type='submit' name='delete' formnovalidate class='btn btn-danger' id='delete-me' onclick="return confirm('<?php e(js_escape(lang('fee_stucture_delete_confirm'))); ?>');">
                    <span class='icon-trash icon-white'></span>&nbsp;<?php echo lang('fee_stucture_delete_record'); ?>
                </button>
            <?php endif; ?>
        </fieldset>
    <?php echo form_close(); ?>
