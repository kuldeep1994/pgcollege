...
<div class='admin-box'>
    ...
    <?php echo form_open($this->uri->uri_string(), 'class="form-horizontal"'); ?>
        <fieldset>
            ...
            <input type="hidden" name="ci_csrf_token" value="<?php echo $this->security->get_csrf_hash(); ?>" id="ci_csrf_token">
        ...
        <fieldset class='form-actions'>
            <input type='submit' name='save' class='btn btn-primary' value="<?php echo lang('school_institute_action_edit'); ?>" />
            <?php echo lang('bf_or'); ?>
            <?php echo anchor(SITE_AREA . '/content/school_institute', lang('school_institute_cancel'), 'class="btn btn-warning"'); ?>
            
            <?php if ($this->auth->has_permission('School_Institute.Content.Delete')) : ?>
                <?php echo lang('bf_or'); ?>
                <button type='submit' name='delete' formnovalidate class='btn btn-danger' id='delete-me' onclick="return confirm('<?php e(js_escape(lang('school_institute_delete_confirm'))); ?>');">
                    <span class='icon-trash icon-white'></span>&nbsp;<?php echo lang('school_institute_delete_record'); ?>
                </button>
            <?php endif; ?>
        </fieldset>
    <?php echo form_close(); ?>
    ...
    $('#country').on('change', function() {
        ...
        if(country_id!='')
        {
            $.ajax({
                    type:'POST',
                    dataType:'json',
                    url: '<?php echo site_url(SITE_AREA) ?>/content/school_institute/getStateByCountryId',
    ...
    $('#state').on('change', function() {
        ...
        if(state_id!='')
        {
            $.ajax({
                    type:'POST',
                    dataType:'json',
                    url: '<?php echo site_url(SITE_AREA) ?>/content/school_institute/getCityByStateId',
