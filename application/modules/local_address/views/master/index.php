...
$num_columns	= 6;
$can_delete	= $this->auth->has_permission('Local_Address.Content.Delete');
$can_edit		= $this->auth->has_permission('Local_Address.Content.Edit');
	...
	<?php echo form_open($this->uri->uri_string()); ?>
		<table class='table table-striped'>
			...
			<tbody>
				...
				<?php if ($can_edit) : ?>
					<td><?php echo anchor(SITE_AREA . '/master/local_address/edit/' . $record->id, '<span class="icon-pencil"></span> ' .  $record->street_address); ?></td>
				<?php else : ?>
					<td><?php e($record->street_address); ?></td>
				<?php endif; ?>
					<td><?php e($this->local_address_model->getCityName($record->city)); ?></td>
					<td><?php e($this->local_address_model->getStateName($record->state)); ?></td>
					<td><?php e($record->zip_code); ?></td>
					<td><?php e($this->local_address_model->getCountryName($record->country)); ?></td>
				</tr>
				<?php
					endforeach;
				else:
